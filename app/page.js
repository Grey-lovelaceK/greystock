...
function EstadoPill({ estado }) {
  if (estado === 'QUIEBRE') {
    return <span className={`${styles.pill} ${styles.pillRed}`}>⚠ QUIEBRE</span>
  }
  if (estado === 'NUEVO') {
    return <span className={`${styles.pill}`} style={{ background:'var(--blue-dim)', color:'var(--blue)', border:'1px solid rgba(59,130,246,0.25)' }}>★ NUEVO</span>
  }
  return <span className={`${styles.pill} ${styles.pillAmber}`}>○ SIN MOV.</span>
}
          ...
          <>
            ...
            <div style={{ marginBottom:16 }}>
              {item.estado === 'QUIEBRE'
                ...
                : item.estado === 'NUEVO'
                ? <span className={`${styles.pill}`} style={{ background:'var(--blue-dim)', color:'var(--blue)', border:'1px solid rgba(59,130,246,0.25)' }}>★ NUEVO — COMPRAR</span>
            ...
            <div className={styles.detailGrid}>
              {[
                ...
                { label:'Sugerencia Compra',  value: `${item.sugerencia_compra ?? '—'} uds`,             color:'var(--accent)' },
    ...
    <div className={styles.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className={styles.panel} style={{ width: 700 }}>
        ...
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: 'var(--muted)', marginBottom: 20 }}>
          {count} SKUs con ventas/compras pero sin ficha de producto.
        </div>
...
export default function Dashboard() {
  ...
  // ── Cruce stock DLDS ──────────────────────────────────────────────────────
  async function fetchDldsStock() {
    setDldsLoading(true)
    setDldsError(null)
    try {
      const res = await fetch('/api/dlds-stock')
      const d = await res.json()
      if (!res.ok) throw new Error(d.error)
      setDldsStock(d.stock || {})
    } catch(e) {
      setDldsError(e.message)
    } finally {
      setDldsLoading(false)
    }
  }

  // ── Descargar Excel sugerencia compra ────────────────────────────────────
  async function downloadExcel() {
    // Traer todos los items del tab actual (sin paginación)
    const allItems = []
    let p = 1
    while (true) {
      const params = new URLSearchParams({ tab, q, tipo, marca, page: p })
      const res = await fetch(`/api/dashboard?${params}`)
      const d = await res.json()
      allItems.push(...(d.data || []))
      if (!d.data?.length || allItems.length >= (d.count || 0)) break
      p++
    }

    const rows = allItems.map(d => {
      // Si hay datos DLDS: sugerencia solo si proveedor tiene stock
      const sugerencia = dldsStock != null
        ? (dldsStock[d.sku] > 0 ? (d.sugerencia_compra ?? 0) : 0)
        : (d.sugerencia_compra ?? 0)
      ...
      const row = {
        ...
        'Rotación Mensual':     Number(d.rotacion || 0).toFixed(2),
        'Ventas 12m':           d.ventas_12m,
        'Precio':               d.precio,
        'Costo Unit.':          d.costo_unit,
        'Margen':               d.margen,
        'Sugerencia Compra (7 días)': sugerencia,
      }
      if (dldsStock != null) {
        row['Stock DLDS'] = dldsStock[d.sku] ?? '—'
      }
      return row
    })
    ...
    ws['!cols'] = [
      { wch: 14 }, { wch: 35 }, { wch: 25 }, { wch: 18 }, { wch: 14 },
      { wch: 12 }, { wch: 12 }, { wch: 16 }, { wch: 12 }, { wch: 12 },
      { wch: 12 }, { wch: 12 }, { wch: 22 }, { wch: 12 },
    ]
    XLSX.utils.book_append_sheet(wb, ws, 'Sugerencia Compra')
    const fecha = new Date().toISOString().split('T')[0]
    XLSX.writeFile(wb, `sugerencia_compra_${fecha}.xlsx`)
  }

  const totalPages = Math.ceil(count / LIMIT)
  const showSugerencia = tab === 'quiebre'
  const colCount = 10 + (showSugerencia ? 1 : 0) + (showSugerencia && dldsStock ? 1 : 0)
    ...
    <div className={styles.app}>
      ...
      {kpis && (
        <div className={styles.kpiStrip}>
          {[
            { label:'Comprar Urgente',    value: kpis.quiebres?.toLocaleString(),      sub:'quiebres + nuevos',      cls:styles.kpiRed   },
      ...
      <div className={styles.toolbar}>
        ...
        {/* Botones acción — solo en tab quiebre */}
        {tab === 'quiebre' && (
          <>
            <button onClick={downloadExcel} className={styles.btnUpdate}
              style={{ background:'var(--green-dim)', borderColor:'rgba(34,197,94,0.3)', color:'var(--green)' }}>
              ↓ Descargar Excel
            </button>
            <button onClick={fetchDldsStock} disabled={dldsLoading} className={styles.btnUpdate}
              style={{ background:'var(--blue-dim)', borderColor:'rgba(59,130,246,0.3)', color:'var(--blue)' }}>
              {dldsLoading ? '⟳ Cargando...' : dldsStock ? `✓ DLDS (${Object.keys(dldsStock).length} SKUs)` : '⟳ Cruzar DLDS'}
            </button>
          </>
        )}
      ...
      {/* Banner estado DLDS */}
      {dldsError && (
        <div style={{ background:'var(--red-dim)', border:'1px solid rgba(255,77,77,0.3)', padding:'8px 20px', fontFamily:"'DM Mono',monospace", fontSize:11, color:'var(--red)' }}>
          ✗ DLDS: {dldsError}
        </div>
      )}
      {dldsStock && !dldsError && (
        <div style={{ background:'var(--green-dim)', border:'1px solid rgba(34,197,94,0.3)', padding:'8px 20px', fontFamily:"'DM Mono',monospace", fontSize:11, color:'var(--green)' }}>
          ✓ Stock DLDS cargado — {Object.keys(dldsStock).length} SKUs. Sugerencias ajustadas por disponibilidad del proveedor.
        </div>
      )}
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          <thead>
            <tr>
              {['Tipo','Producto','SKU','Stock','Rotación','Ventas 12m','Tendencia','Precio','Margen',
                ...(showSugerencia ? ['Sugerencia'] : []),
                ...(showSugerencia && dldsStock ? ['Stock DLDS'] : []),
                'Estado'
              ].map(col => (
                <th key={col}>{col}</th>
              ))}
          ...
          <tbody>
            {items.map(d => {
              const sugerencia = dldsStock != null
                ? (dldsStock[d.sku] > 0 ? (d.sugerencia_compra ?? 0) : 0)
                : (d.sugerencia_compra ?? 0)
              const sinStockDlds = dldsStock != null && !(dldsStock[d.sku] > 0)
              ...
              return (
                <tr key={d.sku} onClick={() => setDetailSku(d.sku)} className={styles.tableRow}>
                  ...
                  {showSugerencia && (
                    <td style={{ fontFamily:"'DM Mono',monospace", textAlign:'right', fontWeight:700,
                      color: sinStockDlds ? 'var(--muted)' : sugerencia > 0 ? 'var(--accent)' : 'var(--muted)' }}>
                      {sinStockDlds ? <span title="Sin stock en DLDS">— (sin prov.)</span> : sugerencia}
                    </td>
                  )}

                  {showSugerencia && dldsStock && (
                    <td style={{ fontFamily:"'DM Mono',monospace", textAlign:'right',
                      color: dldsStock[d.sku] > 0 ? 'var(--green)' : 'var(--red)' }}>
                      {dldsStock[d.sku] != null ? dldsStock[d.sku] : '—'}
                    </td>
                  )}

                  <td><EstadoPill estado={d.estado} /></td>
                </tr>
              )
            })}
            {!loading && items.length===0 && (
              <tr><td colSpan={colCount} style={{ textAlign:'center', padding:40, color:'var(--muted)', fontFamily:"'DM Mono',monospace", fontSize:12 }}>
                Sin resultados
              </td></tr>
            )}
          </tbody>
